...
export default class LevelSettingForm extends FormPage {
  ...
  render() {
    ...
    let validRules = item.id ?
      [{ max: 15, message: "最多输入15个字！" }, { validator: customRules.remote, value: '/sysPositionLevel/findNameIsExist', name: "positionName", id: item.id , defaultValue:item.positionName}, { required: true, message: `职位级别不可为空`,  }, ]
      :
      [{ max: 15, message: "最多输入15个字！" }, { validator: customRules.remote, value: '/sysPositionLevel/findNameIsExist', name: "positionName" }, { required: true, message: `职位级别不可为空` },]
