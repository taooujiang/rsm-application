...
class FeedbackType extends Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (JSON.stringify(nextProps.dataSource) !== JSON.stringify(this.props.dataSource)) {
			this.setState({questionAnswerDtoList: dataSource});
		}
	}
...
class FeedbackForm extends Component {
	...
	setObjFn(array, value) {
		...
		this.setState({
			...
			interviewer: interviewer || (
				array[0]
				? array[0].interviewerId
				: "")
		})
		if (array[0]) {
			this.formSetFields(
				json
				? json.questionAnswerDtoList
				: array[0].questionAnswerDtoList)
		}
	}
	...
	formSetFields(questionAnswerDtoList) {
		...
		let questionList = questionAnswerDtoList.map(it => {
	...
	render() {
		...
		return (<BaseForm onSubmit={handleSubmit} ref={saveFormRef}>
			...
			<FormItem>
				<Select label="面试官" name="notUsed" defaultValue={obj.interviewer
						? obj.interviewer
						: obj.interviewerId} fetch={info} renderItem={this.renderInterviewerOption} onChange={this.handleChange.bind(this)} rules={[
						{
							required: true,
							message: "面试官不可为空"
						}, {
