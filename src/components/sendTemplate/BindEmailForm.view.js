...
class EmailForm extends Component {
	...
	render() {
		...
		return (<BaseForm onSubmit={handleSubmit} ref={this.formRefs.bind(this)}>
			...
			<FormItem {...formFullItemLayout}>
				<Input label="密码" type="password" name="password" rules={[{
							required: true,
							message: "密码不可为空"
						}
					]}/>
			...
			<FormItem {...formFullItemLayout}>
				<Input label="邮箱发送服务器" name="mailHost" rules={[{
							required: true,
							message: "邮箱服务器不可为空"
						}
					]}/>
			...
			<FormItem {...formFullItemLayout} className="input-button-group">
				{
					buttonChanged
						? <Input addonAfter={<CountdownButton name = "重新获取" time = {
									60
								}
								buttonChanged = {
									buttonChanged
								...
								clickEvent = {
									configAccountPwd
								} />} label="验证码" type="text" name="code" rules={[{
										required: true,
										message: "验证码不可为空"
									}
								]}/>
						: <Input addonAfter={<Button onClick = {
									configAccountPwd
								}
								disabled = {
									disabled
								...
								</Button>} label="验证码" type="text" name="code" rules={[{
										required: true,
										message: "验证码不可为空"
									}
								]}/>
...
class EmailFormView extends FormPage {
	...
	sendBindCode() {
		return new API().fetchSendBindCode({email: this.form.getFieldValue('email'), password: this.form.getFieldValue('password')}).then(json => {
			message.success('验证码已发送！')
		})
	}
	...
	handlerCountdown() {
		...
		let timer = setInterval(function() {
			...
			if (time < 0) {
				clearInterval(timer)
				self.setState({content: "重新获取", disabled: false, buttonChanged: true})
			}
		}, 1000)
	}
