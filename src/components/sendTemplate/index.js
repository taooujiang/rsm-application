...
export default class EmailTemplateLinkage extends Component {
	...
	getEmailConfig() {
		new API().fetchEmailConfig().then(json => {
			console.log("then")
			if (json.status == 0) {
				this.setState({textVisible: false, email: json.email})
			} else {
				// console.log("未绑定")
				this.setState({textVisible: true, visible: true})
			}
		}).catch(ex => {
			return "error"
		})
	...
	renderEmailSelectOption(data, idx) {/* data.type = 1 为短信 2为邮件 */
		// console.log(data.id)
		return data.type == 2
			? (<Select.Option value={data.content} key={data.id}>{data.name}</Select.Option>)
			: ""
	}
	...
	hideUnbindModal() {
		this.setState({unbindVisible: false})
	}
	renderBindEmailForm() {
		let {visible} = this.state
		return (
			visible
			? <EmailFormView hideModal={this.hideModal.bind(this)} getEmailConfig={this.getEmailConfig.bind(this)} mailSubject="offer邮件通知"></EmailFormView>
			: null)
	}
	renderUnBindEmailForm() {
		...
		return (
			unbindVisible
			? <UnbindEmailFormView hideModal={this.hideUnbindModal.bind(this)} getEmailConfig={this.getEmailConfig.bind(this)} email={email} mailSubject="offer邮件通知"></UnbindEmailFormView>
			: null)
	}
	...
	render() {
		...
		return (<div>
			...
			<Row>
				<Col span={24}>
					<FormItem {...formFullItemLayout}>
						<Input label="发件人" name="mailFrom" disabled={true} defaultValue={this.state.email} rules={[
								{
									required: true,
									message: "发件人不能为空"
								}, {
						...
						textVisible
							? (<Remote url="">
								<div style={{
									...
									<span style={{
											color: 'red'
										}}>未绑定邮箱</span>，<a onClick={this.showModal.bind(this)}>点击绑定</a>邮箱</div>
							</Remote>)
							: (<Remote url="">
								<div style={{
										paddingLeft: '100px',
									...
									<span style={{
											color: 'red'
										}}>已绑定邮箱</span>，<a onClick={this.showUnbindModal.bind(this)}>点击更改绑定</a>邮箱</div>
							</Remote>)
					}
				</Col>
			</Row>
			<Row>
				<Col span={24}>
					<FormItem {...formFullItemLayout}>
						<Input label="收件人" name="mailTo" defaultValue={mailTo} rules={[
								{
									required: true,
									message: "收件人不能为空"
								}, {
				...
				<Col span={24}>
					<FormItem {...formFullItemLayout}>
						<Select label="邮件模版" name="groupName" onChange={this.templateChange.bind(this)} renderItem={this.renderEmailSelectOption} fetch={`${APP_SERVER}/template/listJson`} params={{
								item: {
									type: "2",
				...
				<Col span={24}>
					<FormItem {...formFullItemLayout}>
						<Input label="邮件主题" name="mailSubject" defaultValue={mailSubject} rules={[
								{
									required: true,
									message: "邮件主题不能为空"
								}, {
	...
	render() {
		...
		return (<div>
			...
			<Row>
				<Col span={24}>
					<FormItem {...formFullItemLayout}>
						<Select label="短信模版" name="groupName1" onChange={this.templateChange.bind(this)} renderItem={this.renderSmsSelectOption} fetch={`${APP_SERVER}/template/listJson`} params={{
								item: {
									type: "1",
									templateUse: templateUse
								}
				...
				<Col span={24}>
					<FormItem {...formFullItemLayout}>
						<Input label="收件人" name="smsMobilephone" defaultValue={receiver} rules={[
								{
									required: true,
									message: "收件人不能为空"
								}, {
	...
	render() {
		...
		return (<div>
			...
			<Row>
				<Col span={12}>
					<FormItem {...formFullItemLayout}>
						<Select label="短信模版" name="groupNameInter" onChange={this.templateChange.bind(this)} renderItem={this.renderSmsSelectOption} fetch={`${APP_SERVER}/template/listJson`} params={{
								item: {
									type: "2",
									templateUse: "1"
								}
