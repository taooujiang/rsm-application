...
function dealWithTags(type, info, bean) {
	if (type == "resume") {
		return cutNullArr([
			DictUtils.getDictLabelByValue("sex", info.sex),
			bean.age + "岁",
...
class TotalItemFrameWork extends Component {
	render() {
		...
		if (title) {
			return (<div>
				<DetailTitle title={title}/> {children}
			</div>)
		} else {
			return (<div>
				{children}
...
class SalaryInfomationShow extends Component {
	renderShowData() {
		...
		return (<TotalItemFrameWork title="目前年收入">
			<Row gutter={12}>
...
class JobWantedInfomationData extends Component {
	render() {
		...
		if (objectives) {
			let expectTotal = !objectives.expectedSalaryUpper && !objectives.expectedSalaryLower
				? ""
				: objectives.expectedSalaryLower + "-" + objectives.expectedSalaryUpper
			let tradeResult = objectives.trade.map((it) => {
				return DictUtils.getDictLabelByValue("industry", it) + ","
			})
			let jobNatureResult = objectives.jobNature.map((it) => {
...
class JobWantedInfomationDataForElite extends Component {
	render() {
		...
		if (info) {
			let expectTotal = !info.expectedSalaryUpper && !info.expectedSalaryLower
				? ""
				: info.expectedSalaryLower + "-" + info.expectedSalaryUpper
			let tradeResult = item.tradeArr && item.tradeArr.map((it) => {
				return DictUtils.getDictLabelByValue("industry", it) + ","
			})
			let jobNatureResult = item.jobNatureArr && item.jobNatureArr.map((it) => {
				return DictUtils.getDictLabelByValue("workproperty", it) + ","
			})
...
class JobWantedInfomationShow extends Component {
	renderShowData() {
		...
		return (<TotalItemFrameWork title="求职意向">
			<Row gutter={12}>
...
class WorkInformationShow extends Component {
	renderShowData() {
		...
		return jobs.map((it, idx) => {
			return (<div key={idx}>
				<Row gutter={12}>
					<LayoutKeyValue span={6} val={<TimeShower type = "array" time = {
							[it.duringStart, it.duringEnd]
						} />}/>
					<LayoutKeyValue span={6} val={<strong> {
							it.company
						}
						</strong>}/>
					<LayoutKeyValue span={6} val={<strong> {
				...
				<Row gutter={12}>
					<LayoutKeyValue span={24} val={cutNullArr([
	...
	render() {
		return (<TotalItemFrameWork title="工作经验">
			{this.renderShowData()}
		</TotalItemFrameWork>)
	}
}
...
class ProjectInfomationShow extends Component {
	...
	render() {
		return (<TotalItemFrameWork title="项目经验">
			{this.renderShowData()}
		</TotalItemFrameWork>)
	}
}
...
class EducationInfomationShow extends Component {
	...
	render() {
		return (<TotalItemFrameWork title="教育经历">
			{this.renderShowData()}
		</TotalItemFrameWork>)
	}
}
...
class LanguageInfomationShow extends Component {
	...
	render() {
		return (<TotalItemFrameWork title="技能/语言">
			{this.renderShowData()}
		</TotalItemFrameWork>)
	}
}
...
class CredentialsInfomationShow extends Component {
	renderShowData() {
		...
		return credentials.map((it, idx) => {
			return (<div key={idx}>
				<Row gutter={12}>
					<LayoutKeyValue span={6} val={<TimeShower type = "string" time = {
							it.getDate
						} />}/>
					<LayoutKeyValue span={10} val={<strong> {
							it.title
						}
						</strong>}/>
					<LayoutKeyValue span={6} val={<strong> {
	...
	render() {
		return (<TotalItemFrameWork title="证书">
			{this.renderShowData()}
		</TotalItemFrameWork>)
	}
}
	...
	renderShowData() {
		...
		return trainings.map((it, idx) => {
			return (<div key={idx}>
				...
				<Row gutter={12}>
					<LayoutKeyValue span={24} val={cutNullArr([it.certificate, it.trainingAddress]).join(" | ")}/>
				</Row>
			</div>)
		})
	}
	render() {
		return (<TotalItemFrameWork title="培训经历">
			{this.renderShowData()}
		</TotalItemFrameWork>)
	}
}
